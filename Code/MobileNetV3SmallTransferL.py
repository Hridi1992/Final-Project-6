...
MODEL_PATH = os.path.abspath('best_model_mob.keras')
# ====================== DATA CONFIGURATION ======================
# Dataset paths
...
def visualize_augmented_images(generator, num_samples=8, cols=4, figsize=(15, 6),
    ...
    plt.savefig(os.path.join(output_dir, "augmented_samples_mob.png"))
...
def build_mobilenet_model():
    ...
    print(f"Total layers in MobileNetV3Small: {len(base_model.layers)}")
...
def train_model(model, train_gen, val_gen,
    ...
    base_model = model.get_layer("mobilenetv3")
    ...
    print("\n=== Phase 2: Fine-Tuning ===")
    unfreeze_schedule = [
        (134, 157, 15, 1e-5),  # Final blocks
        (64, 134, 20, 5e-6),  # Middle blocks
        (10, 64, 25, 1e-6)  # Early blocks
    ]
...
def visualize_conv_kernels(model, layer_name, layer_index, max_filters=32, output_dir='conv_kernels'):
    ...
    if max_filters == -1:
        ...
        # Save visualization
        os.makedirs(output_dir, exist_ok=True)
        filename = f"{layer.name}_kernels_mob.png"
...
def plot_feature_maps(model, input_image, layer_index=0, rows=4, cols=8,
    ...
    if filename is None:
        filename = f"layer_{layer_index}_{layer.name}_feature_maps_mob.png"
...
def plot_mobile_feature_maps(model, input_image, rows=4, cols=8,
    ...
    resnet = model.get_layer("mobilenetv3")
    ...
    if not resnet_conv_layers:
        raise ValueError("No Conv2D layers found in MobileNetV3Small")
    ...
    for layer, fmaps in zip(resnet_conv_layers, feature_maps):
        ...
        # Save and close
        plt.tight_layout()
        filename = f"{filename_prefix}{layer.name}_feature_maps_mob.png"
        plt.savefig(os.path.join(output_dir, filename), bbox_inches='tight')
        plt.close()
...
def plot_combined_history(history, output_dir='training_history'):
    ...
    plt.tight_layout()
    os.makedirs(output_dir, exist_ok=True)
    plt.savefig(os.path.join(output_dir, "training_history_mob.png"))
    plt.close()
...
def evaluate_model(model, test_gen, class_names, output_dir='results'):
    ...
    plt.ylabel("Recall")
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "recall_distribution_mob.png"))
    plt.close()
    ...
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "confusion_matrix_mob.png"))
    plt.close()
...
def main():
    ...
    visualize_augmented_images(
        ...
        output_dir='augmentedImages_mob'  # Save to dedicated folder
    )

    # ====================== MODEL SETUP ======================
    # Construct model architecture
    model = build_mobilenet_model()  # Custom ResNet-based architecture
    ...
    print("\n=== MobileNetV3Small Layer Names (First 5 Layers) ===")
    base_model = best_model.get_layer("mobilenetv3")  # Access base architecture
    ...
    visualize_conv_kernels(
        model=base_model,
        layer_name='Conv',  # Specific EffcientNetB3 layer
        layer_index=-1,  # Legacy mode indicator
        max_filters=128,  # Limit displayed filters
        output_dir='kernels_mob'  # Save to kernels directory
    )
    ...
    plot_mobile_feature_maps(
        model=best_model,
        input_image=sample_image,
        output_dir='featureMaps_mob',  # Unified output directory
    ...
    plot_combined_history(
        combined_history,
        output_dir='results_mob'  # Save visualization to results/
    )
