...
# Suppress TensorFlow warnings (addresses "Skipping the delay kernel" warning)
import os
...
from tensorflow.keras.applications.resnet_v2 import preprocess_input  # Changed to ResNet50V2 preprocessing
...
# Configure GPU memory growth to prevent pre-allocation of all GPU memory
...
TARGET_SIZE = 224  # Required input size for ResNet50V2
...
def resnet50v2_preprocess(x):  # Renamed to resnet50v2_preprocess
    """Custom preprocessing function for ResNet50V2 compatibility"""
...
def create_data_generators():
    ...
    train_datagen = ImageDataGenerator(
        ...
        rotation_range=25,  # Random rotations ±25 degrees
...
# ====================== MODEL ARCHITECTURE ======================
def build_resnet50v2_model():  # Renamed to build_resnet50v2_model
    """Build hybrid CNN-Transformer architecture with ResNet50V2"""
    ...
    x = layers.Lambda(resnet50v2_preprocess)(x)  # Updated to resnet50v2_preprocess

    # ========== BASE MODEL ==========
    # Initialize pre-trained ResNet50V2 without top layers
    base_model = ResNet50V2(
        weights='imagenet',  # Pre-trained on ImageNet
        include_top=False,  # Exclude classification layers
        input_shape=(TARGET_SIZE, TARGET_SIZE, 3),
        pooling=None,  # Add global average pooling
        name='resnet50v2'  # Updated name to resnet50v2
    )
    x = base_model(x)  # Pass through ResNet50V2

    print(f"Total layers in ResNet50V2: {len(base_model.layers)}")
...
def train_model(model, train_gen, val_gen, initial_epochs=1, fine_tune_epochs=1):
    ...
    print("\n=== Initial Training ===")
    model.get_layer("resnet50v2").trainable = False  # Updated to resnet50v2
    model.compile(
    ...
    # Phase 2: Fine-tuning
    print("\n=== Fine-Tuning ===")
    base_model = model.get_layer("resnet50v2")  # Updated to resnet50v2
    base_model.trainable = True

    # Unfreeze last 25% of layers
    num_layers = len(base_model.layers)
    for layer in base_model.layers[:int(0.75 * num_layers)]:  # Freeze first ~142 layers
...
def plot_resnet50v2_feature_maps(model, input_image, layer_name=None, rows=4, cols=8):  # Renamed to plot_resnet50v2_feature_maps
    """
    Visualize feature maps from ResNet50V2's convolutional layers.
    """
    # 1. Get ResNet50V2 base model
    resnet = model.get_layer("resnet50v2")  # Updated to resnet50v2

    # 2. Get all convolutional layers from ResNet50V2
    resnet_conv_layers = [layer for layer in resnet.layers
                          if isinstance(layer, layers.Conv2D)]

    if not resnet_conv_layers:
        raise ValueError("No convolutional layers found in ResNet50V2")
    ...
    processed_img = resnet50v2_preprocess(processed_img)  # ResNet50V2 preprocessing

    # 4. Create direct access to ResNet50V2
...
def main():
    ...
    model = build_resnet50v2_model()  # Updated to build_resnet50v2_model
    ...
    best_model = models.load_model('best_model.keras', custom_objects={'resnet50v2_preprocess': resnet50v2_preprocess})  # Updated to resnet50v2_preprocess

    # Get the ResNet50V2 base model from your architecture
    print("\n=== ResNet50V2 Layer Names ===")
    base_model = best_model.get_layer("resnet50v2")  # Updated to resnet50v2
    for layer in base_model.layers[:5]:  # First 5 layers
        print(layer.name)

    # Visualize kernels from ResNet50V2 and custom conv layer
    visualize_conv_kernels(base_model, 'conv1_conv', -1, max_filters=128)  # Updated to valid ResNet50V2 layer
    visualize_conv_kernels(model, 'conv2d', layer_index=0, max_filters=-1)  # Updated to use proper layer name
    ...
    # Visualize all ResNet50V2 conv layers
    plot_resnet50v2_feature_maps(best_model, sample_image)  # Updated to plot_resnet50v2_feature_maps
